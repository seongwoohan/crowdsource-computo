...
transformed parameters {
  real log_lik;
  vector[J] logit_alpha_sens = rep_vector(alpha_acc_scalar, J);
  vector[J] logit_alpha_spec = rep_vector(alpha_acc_scalar, J);

  {
    ...
    for (n in 1:N) {
      ...
      lp_pos[i] += bernoulli_lpmf(
        y | inv_logit(logit_alpha_sens[j])
      );
      lp_neg[i] += bernoulli_lpmf(
        y | inv_logit(-logit_alpha_spec[j])
      );
    }
